 ...
 * 虚拟机调度分时系统是一个虚拟机监控器分配策略，用来从一个PM分配一个或者多个PEs到另一个虚拟机，并且允许通过复合的虚拟机进行分享。
 * 这个类也产生因为虚拟机迁移导致的10%的性能退化。这个调度不支持过载
 * 
 * Each host has to use its own instance of a VmScheduler
 * that will so schedule the allocation of host's PEs for VMs running on it.
 * 每个主机必须使用它的虚拟机调度算法实例来调度，分配跑在主机上的虚拟机的主机的PEs
 * 
...
public class VmSchedulerTimeShared extends VmScheduler {
	...
	public boolean allocatePesForVm(Vm vm, List<Double> mipsShareRequested) {
		/*
		 * @todo add the same to RAM and BW provisioners 添加RAM和BW供应者
		 */
		if (vm.isInMigration()) {
	...
	protected boolean allocatePesForVm(String vmUid, List<Double> mipsShareRequested) {
		...
		for (Double mips : mipsShareRequested) {
			// each virtual PE of a VM must require not more than the capacity of a physical PE 每个虚拟机的虚拟PE必须需求不超过物理PE的能力
			if (mips > peMips) {
				return false;
			}
		...
		// This scheduler does not allow over-subscription 调度算法不允许过载
		...
		if (getVmsMigratingIn().contains(vmUid)) {
			// the destination host only experience 10% of the migrating VM's MIPS 目标主机只有迁移虚拟机的mips的10%
		...
		for (Double mipsRequested : mipsShareRequested) {
			if (getVmsMigratingOut().contains(vmUid)) {
				// performance degradation due to migration = 10% MIPS 因为迁移导致的性能退化
				mipsRequested *= 0.9;
	...
	/**
	 * Update allocation of VMs on PEs. 更新关于PE的分配
