 ...
 * 这个类拓展了CloudSimCore，从而在Cloudsim中实现网络仿真。
 * 同时，它禁用了所有CloudSim中的网络模式，来提供一个更简单的网络仿真。
 * 在Cloudsim中使用的网络模型，用BRITE(基速接口传输扩展)格式写的拓扑文件用于描述网络。
 * 之后，这样的文件中的节点被映射到Cloudsim实体中。
 * 延迟计算从BRITE模型添加到通过CloudSim发送的消息。
 * 使用旧的模型的消息被转变成使用正确参数的合适的方法
 * 
...
public class CloudSim {
	...
	/** The Constant CLOUDSIM_VERSION_STRING. 常量——Cloudsim版本*/
	private static final String CLOUDSIM_VERSION_STRING = "3.0";

	/** The id of CIS entity. CIS实体ID*/
	private static int cisId = -1;

	/** The id of CloudSimShutdown entity. CloudsimShutdown实体编号*/
	@SuppressWarnings("unused") //表示可以不使用，取消相关的警告
	private static int shutdownId = -1;

	/** The CIS object. CIS对象*/
	private static CloudInformationService cis = null;

	/** The Constant NOT_FOUND. 常量——未找到*/
	 ...
	 * 事件之间的最小事件。在最后一个事件之后的较短时间内的事件被丢弃。
	 * 
	 * */
	 ...
	 * 初始化Cloudsim参数，这个方法应当在创建任何实体前调用
	 * 
	 * <p>
	 ...
	 * future event queue.
	 * 开始Cloudsim仿真运行。
	 * 它等待所有实体的执行完成，即直到所有的实体线程到达non-RUNNABLE状态或者在future队列中没有更多的事件
	 * 
	 * <p>
	 ...
	 * execution.
	 * 停止云仿真。
	 * 在运行中，只有任何在试题中定义的使用者想要终止仿真才能被调用
	 * 
	 * @throws NullPointerException This happens when creating this entity before initialising
	 *             CloudSim package or this entity name is <tt>null</tt> or empty
	 * @see gridsim.CloudSim#init(int, Calendar, boolean)
	...
	public static int waiting(int d, Predicate p) {
		...
		while (iterator.hasNext()) {
			event = iterator.next();
			if ((event.getEntDst() == d) && (p.match(event))) {
				count++;
			}
		}
		return count;
	...
	public static SimEvent select(int src, Predicate p) {
		...
		while (iterator.hasNext()) {
			ev = iterator.next();
			if (ev.getEntDst() == src && p.match(ev)) {
				iterator.remove();
				break;
			}
		}
	...
	public static SimEvent cancel(int src, Predicate p) {
		...
		while (iter.hasNext()) {
			ev = iter.next();
			if (ev.getEntSrc() == src && p.match(ev)) {
				iter.remove();
				break;
			}
		}
				...
				if (dest < 0) {
					...
					if (dest_ent.getState() == SimEntity.WAITING) {
						...
						if ((p == null) || (tag == 9999) || (p.match(e))) {
							dest_ent.setEvbuf((SimEvent) e.clone());
		...
		while (true) {
			...
			if (pauseAt != -1
					&& ((future.size() > 0 && clock <= pauseAt && pauseAt <= future.iterator().next()
							.getTime()) || future.size() == 0 && pauseAt <= clock)) {
				pauseSimulation();
				clock = pauseAt;
			}
