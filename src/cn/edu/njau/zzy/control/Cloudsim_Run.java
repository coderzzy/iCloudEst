...
/**
 * 利用方法接口和仿真父类实现的一个云仿真子类(example1-6,静态部署)
 * 
 * @author zzy
 *
 */
public class Cloudsim_Run extends CloudSimulatedControl implements CloudsimulatedFunction{
	...
	// 构造函数
	public Cloudsim_Run(Map<String, Object> session){
	...
	public boolean execute(HttpServletRequest req, Map<String, Object> session) {
		...
		try {
			...
			// 获得调度算法有关的前台数据
			int AlgorithmChoose = Integer.parseInt(req.getParameter("algorithm"));  // 调度算法选择,选择值下标从0开始
			...
			if (AlgorithmChoose == 1) {
				session.put("suanfa", "RR轮询算法");
				result.append("选择的算法:RR轮询算法\r\n");
				result_data.append("1\r\n");
				broker.bindCloudletsToVmsSimple();
			} else if (AlgorithmChoose == 3){
				session.put("suanfa", "改进的Min-min贪心算法");
				result.append("选择的算法:改进的Min-min贪心算法\r\n");
				result_data.append("3\r\n");
				broker.bindCloudletsToVmsTimeAwared();
			} else if (AlgorithmChoose == 2) {
				session.put("suanfa", "Min-min(极小算法)");
				result.append("选择的算法:Min-min(极小算法)\r\n");
				result.append("2\r\n");
				broker.bindCloudletsToVmsTimeMin_min();
			} else if (AlgorithmChoose == 4){
				session.put("suanfa", "Max-min fairness(最大最小公平算法)");
				result.append("选择的算法:Max-min fairness(最大最小公平算法)\r\n");
				result_data.append("4\r\n");
				broker.bindCloudletsToVmsTimeMax_min();
			} else {
				session.put("suanfa", "RR轮询算法");
				result.append("选择的算法:RR轮询算法\r\n");
				result_data.append("1\r\n");
